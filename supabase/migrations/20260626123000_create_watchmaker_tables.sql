...
-- This migration is intentionally idempotent because earlier SQL files lived
-- outside Supabase's standard migrations directory and may have been applied manually.

create table if not exists public.watchmakers (
  id uuid primary key default gen_random_uuid(),
  name text not null,
  slug text,
  profile_type text not null default 'community',
...
alter table public.watchmaker_claim_requests
  drop constraint if exists watchmaker_claim_requests_status_check,
  add constraint watchmaker_claim_requests_status_check
...
grant select, insert on public.watchmakers to anon, authenticated;
grant update on public.watchmakers to authenticated;
grant select, insert on public.watchmaker_service_reports to anon, authenticated;
grant select, insert on public.watchmaker_claim_requests to authenticated;
grant select on public.watchmaker_claim_reviewers to authenticated;
