...
function XFallbackCard({ href, username }: { href: string; username: string }) {
  return (
    <a
      href={href}
      target="_blank"
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="truncate text-xs text-quiet-muted">{href}</p>
      </div>
...
function normalizeTweetForReactTweet(tweet: any): any {
  ...
  const media = Array.isArray(rawEntities.media) && rawEntities.media.length > 0
    ? rawEntities.media
    : undefined
  ...
  return {
    ...
    quoted_tweet: tweet.quoted_tweet ? normalizeTweetForReactTweet(tweet.quoted_tweet) : undefined,
  }
}
...
export function XEmbed({ url }: { url: string }) {
  ...
  const tweetId = parsed?.canonicalUrl.match(/\/status\/(\d+)/)?.[1] ?? null
  const { data: tweet, error, isLoading } = useTweet(tweetId ?? undefined)
  const normalizedTweet = useMemo(() => normalizeTweetForReactTweet(tweet), [tweet])

  if (!parsed || !tweetId) return null

  const fallback = <XFallbackCard href={parsed.canonicalUrl} username={parsed.username} />

  const components: TweetComponents = {
    TweetNotFound: () => fallback,
  }

  if (isLoading) return <TweetSkeleton />
  if (error || !normalizedTweet) return fallback

  return (
    <div className="mt-3 [&_.react-tweet-theme]:!my-0 [&_article]:!mx-0">
      <TweetRenderBoundary fallback={fallback}>
        <EmbeddedTweet tweet={normalizedTweet} components={components} />
      </TweetRenderBoundary>
    </div>
  )
}
