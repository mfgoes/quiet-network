...
export function scorePost(post: Post, sessionSeed: number): number {
  ...
  return recency + engagement + jitter - penalty
}

// Welcome posts pinned at top, everything else scored and shuffled
export function sortPostsWithFreshness(posts: Post[], sessionSeed: number): Post[] {
  if (posts.length === 0) return []
  const filtered = posts.filter(p => !p.is_welcome)
  filtered.sort((a, b) => scorePost(b, sessionSeed) - scorePost(a, sessionSeed))
  return filtered
}
